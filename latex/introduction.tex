...
\label{datasetintro}
I did not manage to find any suitable existing dataset for this project. For this reason, I personally scraped and tagged $336$ sentences extracted from posts of a medical forum named ``Doctors Lounge'' \cite{doctorslounge}. In section \ref{sec:dataset} I will describe the process I followed in order to create this dataset.

\section{Brief description of results}
\label{sec:brief_results}
As we will see in chapter \ref{cha:results}, BERT is better than R-NET in identifying symptoms. Using BERT, the model can identify correctly the $84.8 \%$ of TFPs, considering correct and redundant TFPs (redundant tokens are, after all, correct); and the $52.6 \%$, considering only correct TFPs. This component, always using BERT, can identify the $88.0 \%$ of symptoms present in the sentences. To deepen the just mentioned measures, read section \ref{sec:eval_symptom_identifier}.

%TODO symptom classifier
